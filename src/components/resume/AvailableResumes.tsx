...
import { Card, Button, Menu, ActionIcon, Text, Group, Stack, rem, ButtonProps } from "@mantine/core";
import { MoreVertical, Edit, Trash2, Download, Clock, ExternalLink } from "lucide-react";
import {
    AvailableResume,
    useGetAvailableResumesSubscription,
            ...
            <Group justify="space-between" align="center">
                <Stack gap={4}>
                    ...
                    <Group gap="xs" c="dimmed">
                        ...
                        <Text size="xs">
                            Last modified:{" "}
                            {format(fromUnixTime(Number(lastModifiedSeconds)), "MMM d, yyyy 'at' h:mm a")}
                        </Text>
                    </Group>
                </Stack>
                ...
                <Group gap="sm">
                    <Button
                        renderRoot={(props: AnchorHTMLAttributes<HTMLAnchorElement>) => (
                            <Link to={"/$resumeId/edit"} params={{ resumeId: id }} {...props} />
                        )}
                        variant="light"
...
export const YourAvailableResumes = () => {
    ...
    const resumes = React.useMemo(() => {
        ...
        return R.pipe(rawResumes, R.sortBy([(x) => Number(x.lastModifiedSeconds), "desc"]));
    ...
    if (resumes.length === 0 && !fetching) {
        return (
            <Card
                ...
                radius="md"
                withBorder
                style={{ borderStyle: "dashed", textAlign: "center", backgroundColor: "transparent" }}
            >
